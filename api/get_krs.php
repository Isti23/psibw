...
$query_mhs = mysqli_query($conn, "SELECT semester FROM mahasiswa WHERE id_user = '$id_user'");

if ($query_mhs && mysqli_num_rows($query_mhs) > 0) {
    $data_mhs = mysqli_fetch_assoc($query_mhs);
    $semester_mhs = $data_mhs['semester']; 
} else {
...
$query = mysqli_query($conn, "
    SELECT 
        ...
        dosen.nama_dosen
    FROM jadwal
    JOIN matakuliah ON jadwal.id_mk = matakuliah.id_mk
    JOIN dosen ON jadwal.id_dosen = dosen.id_dosen
    WHERE matakuliah.semester = '$semester_mhs'
");
